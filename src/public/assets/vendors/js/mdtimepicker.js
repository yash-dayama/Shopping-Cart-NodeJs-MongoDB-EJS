...
(function ($) {
  $.fn.clockTimePicker = function (options, _value) {
    ...
    return this.each(function () {
        ...
        if (!$(this).parent().hasClass("clock-timepicker"))
          ...
          if (options == "dispose") disposeTimePicker($(this));
            ...
            var mobileInput = $(this)
              .parent()
              .find(".clock-timepicker-mobile-input");
      ...
      var popupWidth = isMobile()
        ? $(document).width() - 80
        : settings.popupWidthOnDesktop;
      ...
      if (isMobile()) {
        popup.css("left", "40px").css("top", "40px");
      ...
      if (!isMobile()) {
        function onWindowClick(event) {
          if (
            ...
            !(
              $(event.target)[0] == inputElement[0] ||
              $.contains(inputElement.parent()[0], $(event.target)[0])
            )
          ) {
            hideTimePicker();
          }
      ...
      if (isMobile()) {
        ...
        var inputElementHours = $(
          '<span class="clock-timepicker-mobile-time-hours">'
        );
        ...
        var inputElementMinutes = $(
          '<span class="clock-timepicker-mobile-time-minutes">'
        );
      ...
      if (element.attr("autocapitalize"))
        element.attr(
          "data-autocapitalize-orig",
          element.attr("autocapitalize")
        );
      ...
      inputElement.on(
        ...
        onInputElementDragSelectContextMenu
      );
      ...
      var clockMinuteCanvas = $(
        '<canvas class="clock-timepicker-minute-canvas">'
      );
      clockMinuteCanvas
        .css("cursor", "default")
        .css("position", "absolute")
      ...
      if (isMobile()) {
        var buttonArea = $("<div>");
        buttonArea.css("text-align", "right").css("padding", "15px 30px");
        settings.fonts.fontFamily = settings.fonts.fontFamily
          .replace(/\"/g, "")
          .replace(/\'/g, ""); //Prevents quotes in font to interfere
        var buttonHtml =
          '<a style="text-decoration:none; color:' +
          settings.colors.buttonTextColor +
          "; font-family:" +
          settings.fonts.fontFamily +
          "; font-size:" +
          settings.fonts.buttonFontSize +
      ...
      function registerDraggingEventsOnCanvas(canvas) {
        ...
        if (!isMobile()) {
          ...
          canvas.on("mouseup", function (event) {
            ...
            var x = event.pageX - $(this).offset().left;
            var y = event.pageY - $(this).offset().top;
            var selectorLength = Math.sqrt(
              Math.pow(Math.abs(x - clockCenterX), 2) +
                Math.pow(Math.abs(y - clockCenterY), 2)
            );
            if (
              settings.duration &&
              settings.durationNegative &&
              selectorLength <= 20
            ) {
      ...
      function processClick(event) {
        ...
        if (!getInputElementValue()) {
          ...
          var elementPaddingRight = parseFloat(
            inputElement.css("padding-right")
          );
          var elementInnerWidth =
            elementWidth - elementPaddingLeft - elementPaddingRight;
          ...
          tempAutosizeElement.html(
            getInputElementValue().replace(
              new RegExp(settings.separator + "[0-9]+$"),
              ""
            )
          );
          ...
          if (
            ...
            (textDirection == "ltr" && textAlignment == "start") ||
            (textDirection == "rtl" && textAlignment == "end")
          ) {
            inputElementCenter = Math.floor(elementPaddingLeft + textCenter);
          } else if (textAlignment == "center") {
            inputElementCenter = Math.floor(
              elementPaddingLeft +
                (elementInnerWidth - textWidth) / 2 +
                textCenter
            );
          } else if (
            textAlignment == "right" ||
            (textDirection == "ltr" && textAlignment == "end") ||
            (textDirection == "rtl" && textAlignment == "start")
          ) {
            inputElementCenter = Math.floor(
              elementPaddingLeft + elementInnerWidth - (textWidth - textCenter)
            );
          }
          if (
            event.offsetX >= inputElementCenter - 2 &&
            (popupVisible || !settings.alwaysSelectHoursFirst)
          ) {
      ...
      function processMouseWheelEvent(event) {
        ...
        var negative =
          settings.duration && settings.durationNegative && RegExp.$1 == "-"
            ? true
            : false;
      ...
      function processTimeSelection(x, y, clicked) {
        var selectorAngle =
          (360 * Math.atan((y - clockCenterY) / (x - clockCenterX))) /
            (2 * Math.PI) +
          90;
        var selectorLength = Math.sqrt(
          Math.pow(Math.abs(x - clockCenterX), 2) +
        ...
        if (selectionMode == "HOUR") {
          ...
          var h = -1;
          if (
            selectorLength < clockRadius + 10 &&
            selectorLength > clockRadius - 28
          ) {
            if (x - clockCenterX >= 0) {
          ...
          if (m > -1) {
            if (settings.precision != 1) {
              ...
              m =
                f * settings.precision +
                (Math.round(
                  (m - f * settings.precision) / settings.precision
                ) == 1
                  ? settings.precision
                  : 0);
              if (m >= 60) m = 0;
      ...
      function repaintClockHourCanvas(hoverHour, hoverSign) {
        ...
        if (!isMobile() && hoverHour) {
          ...
          if (isMinMaxFullfilled) {
            ...
            ctx.arc(
              clockCenterX +
                Math.cos((Math.PI / 6) * ((hoverHour % 12) - 3)) *
                  (hoverHour > 12
                    ? settings.afternoonHoursInOuterCircle
                      ? clockOuterRadius
                      : clockInnerRadius
                    : settings.afternoonHoursInOuterCircle
                    ? clockInnerRadius
                    : clockOuterRadius),
              clockCenterY +
                Math.sin((Math.PI / 6) * ((hoverHour % 12) - 3)) *
                  (hoverHour > 12
                    ? settings.afternoonHoursInOuterCircle
                      ? clockOuterRadius
                      : clockInnerRadius
        ...
        if (
          ...
          (!settings.maximum ||
            hour == 24 ||
            isTimeSmallerOrEquals(hour, settings.maximum))
        ) {
          ctx.beginPath();
          ctx.moveTo(clockCenterX, clockCenterY);
          ctx.lineTo(
            clockCenterX +
              Math.cos((Math.PI / 6) * ((hour % 12) - 3)) *
                (hour > 12
                  ? settings.afternoonHoursInOuterCircle
                    ? clockOuterRadius
                    : clockInnerRadius
        ...
        ctx.font =
          settings.fonts.clockOuterCircleFontSize +
          "px " +
          settings.fonts.fontFamily;
        for (i = 1; i <= 12; i++) {
          ...
          if (
            ...
            ctx.fillText(
              s,
              clockCenterX +
                Math.cos(angle) * clockOuterRadius -
                ctx.measureText(s).width / 2,
              clockCenterY +
                Math.sin(angle) * clockOuterRadius +
                settings.fonts.clockOuterCircleFontSize / 3
            );
          } else if (!settings.hideUnselectableNumbers) {
            ctx.fillStyle =
              settings.colors.clockOuterCircleUnselectableTextColor;
            ctx.fillText(
              s,
              clockCenterX +
                Math.cos(angle) * clockOuterRadius -
        ...
        ctx.font =
          settings.fonts.clockInnerCircleFontSize +
          "px " +
          settings.fonts.fontFamily;
        for (i = 1; i <= 12; i++) {
          var angle = (Math.PI / 6) * (i - 3);
          ...
          if (
            ...
            ctx.fillText(
              s,
              clockCenterX +
                Math.cos(angle) * clockInnerRadius -
                ctx.measureText(s).width / 2,
              clockCenterY +
                Math.sin(angle) * clockInnerRadius +
                settings.fonts.clockInnerCircleFontSize / 3
            );
          } else if (!settings.hideUnselectableNumbers) {
            ctx.fillStyle =
              settings.colors.clockInnerCircleUnselectableTextColor;
            ctx.fillText(
              s,
              clockCenterX +
                Math.cos(angle) * clockInnerRadius -
      ...
      function repaintClockMinuteCanvas(hoverMinute, hoverSign) {
        ...
        if (!isMobile() && hoverMinute) {
          ...
          if (isMinMaxFullfilled) {
            ...
            ctx.arc(
              clockCenterX +
                Math.cos((Math.PI / 6) * (hoverMinute / 5 - 3)) *
                  clockOuterRadius,
              clockCenterY +
                Math.sin((Math.PI / 6) * (hoverMinute / 5 - 3)) *
                  clockOuterRadius,
              15,
              0,
              2 * Math.PI,
        ...
        if (
          min > -1 &&
          (!settings.maximum ||
            isTimeSmallerOrEquals(hour + ":" + min, settings.maximum)) &&
          (!settings.minimum ||
            isTimeSmallerOrEquals(settings.minimum, hour + ":" + min))
        ) {
          ctx.beginPath();
          ctx.moveTo(clockCenterX, clockCenterY);
          ctx.lineTo(
            clockCenterX +
              Math.cos((Math.PI / 6) * (min / 5 - 3)) * clockOuterRadius,
            clockCenterY +
              Math.sin((Math.PI / 6) * (min / 5 - 3)) * clockOuterRadius
          );
          ctx.lineWidth = 1;
          ctx.strokeStyle = settings.colors.selectorColor;
          ctx.stroke();
        ...
        for (i = 1; i <= 12; i++) {
          if (
            Math.floor((i * 5) / settings.precision) !=
            (i * 5) / settings.precision
          )
            continue;
          var angle = (Math.PI / 6) * (i - 3);
      ...
      function adjustMobilePopupDimensionAndPosition() {
        ...
        if (window.innerHeight < 400) {
          ...
          popupHeight =
            popupWidth +
            parseInt(canvasHolder.css("margin-top")) +
            parseInt(canvasHolder.css("margin-bottom"));
        }
        ...
        popup.css(
          "left",
          parseInt(($("body").prop("clientWidth") - popup.outerWidth()) / 2) +
            "px"
        );
        popup.css(
          "top",
          parseInt((window.innerHeight - popupHeight) / 2) + "px"
        );
      ...
      function positionPopup() {
        ...
        if (top + popup.outerHeight() > window.innerHeight) {
          var newTop =
            element.offset().top - $(window).scrollTop() - popup.outerHeight();
          if (newTop >= 0) top = newTop;
        }
        var left =
          element.offset().left -
          $(window).scrollLeft() -
          parseInt((popup.outerWidth() - element.outerWidth()) / 2);
      ...
      function hideTimePicker() {
        ...
        if (
          !isTimeChanged &&
          !oldValue &&
          newValue.match(new RegExp("^0+" + settings.separator + "00$"))
        ) {
      ...
      function selectHourOnInputElement() {
        ...
        setTimeout(function () {
          if (isMobile()) {
            $(".clock-timepicker-mobile-time-hours").css(
              "backgroundColor",
              "rgba(255, 255, 255, 0.6)"
            );
            $(".clock-timepicker-mobile-time-minutes").css(
              "backgroundColor",
              "inherit"
            );
          } else {
            inputElement
              ...
              .setSelectionRange(
                0,
                getInputElementValue().indexOf(settings.separator)
              );
          }
        }, 1);
      }
        ...
        setTimeout(function () {
          if (isMobile()) {
            ...
            inputElement
              .get(0)
              .setSelectionRange(
                getInputElementValue().indexOf(settings.separator) + 1,
                getInputElementValue().length
              );
          }
        }, 1);
      }
      ...
      function autosize() {
        ...
        element.css(
          "width",
          tempAutosizeElement.outerWidth() +
            5 +
        ...
        if (new RegExp("^(-|\\+)?([0-9]+)(.([0-9]{1,2})?)?$", "i").test(time)) {
          ...
          if (settings.precision != 1) {
            ...
            min =
              f * settings.precision +
              (Math.round(
                (min - f * settings.precision) / settings.precision
              ) == 1
                ? settings.precision
                : 0);
            if (min == 60) {
          ...
          time =
            "0" + (!settings.duration ? "0" : "") + settings.separator + "00";
        }
        return (
          (settings.duration &&
          settings.useDurationPlusSign &&
          !time.match(/^\-/) &&
          !time.match(/^0+:00$/)
            ? "+"
            : "") + time
      ...
      function disposeTimePicker(element) {
        ...
        if (element.attr("data-autocapitalize-orig")) {
          element.attr(
            "autocapitalize",
            element.attr("data-autocapitalize-orig")
          );
      ...
      function onInputElementKeyDown(event) {
        ...
        //ARROW LEFT OR HOME
          ...
          (event.keyCode == 36 || event.keyCode == 37) &&
          getInputElementValue() != ""
        ) {
        ...
        //ARROW UP OR "+" SIGN, ARROW DOWN OR "-" SIGN
          ...
          var m = RegExp.$4 ? parseInt(RegExp.$4) : 0;
          if (selectionMode == "HOUR")
            h += event.keyCode == 38 || event.key == "+" ? 1 : -1;
          else {
            m +=
              (event.keyCode == 38 || event.key == "+" ? 1 : -1) *
              settings.precision;
            if (m < 0) m = 0;
          ...
          if (
            (!settings.duration || !settings.durationNegative) &&
            min[0] == "-"
          )
            min = "0:00";
          ...
          if (settings.precision != 1) {
            ...
            max =
              max.replace(/.[0-9]+$/, "") +
              settings.separator +
              (minPart - (minPart % settings.precision));
          }
          var newVal =
            (h < 0 ? "-" : "") +
            (h < 10 && !settings.duration ? "0" : "") +
      ...
      function onInputElementKeyUp(event) {
        ...
        var newVal =
          (selectionMode == "HOUR"
            ? (isNegative ? "-" : "") +
              (!settings.duration && enteredDigits.length == 1 ? "0" : "") +
              enteredDigits
            : hourPart) +
          settings.separator +
          (selectionMode == "HOUR"
            ? minPart
            : (enteredDigits.length == 1 ? "0" : "") + enteredDigits);
        if (isTimeSmallerOrEquals(newVal, settings.minimum))
          newVal = settings.minimum;
        if (isTimeSmallerOrEquals(settings.maximum, newVal))
        ...
        var nextPossibleVal =
          (selectionMode == "HOUR"
            ? (isNegative ? "-" : "") + (enteredDigits + "0")
            : hourPart) +
          settings.separator +
          (selectionMode == "HOUR" ? "00" : enteredDigits + "0");
        if (
          (selectionMode == "MINUTE" &&
            (enteredDigits.length == 2 ||
              parseInt(enteredDigits + "0") >= 60)) ||
          (selectionMode == "HOUR" &&
            !settings.duration &&
            enteredDigits.length == 2) ||
          (isNegative
            ? !isTimeSmallerOrEquals(settings.minimum, nextPossibleVal)
            : !isTimeSmallerOrEquals(nextPossibleVal, settings.maximum))
        ) {
          enteredDigits = "";
          if (selectionMode == "HOUR") {
            if (
              settings.precision == 60 ||
              (newVal == settings.maximum && settings.maximum.match(/00$/)) ||
              (settings.minimum[0] == "-" &&
                newVal == settings.minimum &&
                settings.minimum.match(/00$/))
            ) {
              hideTimePicker();
              return;
            } else {
      ...
      function getInputElementValue() {
        if (isMobile()) {
          return (
            $(".clock-timepicker-mobile-time-hours").html() +
            settings.separator +
            $(".clock-timepicker-mobile-time-minutes").html()
          );
        } else {
      ...
      function setInputElementValue(value) {
        if (isMobile()) {
          if (value.match(/^(-|\\+)?([0-9]{1,2}).([0-9]{1,2})$/)) {
            var hours =
              RegExp.$1 +
              (!settings.duration && RegExp.$2.length == 1 ? "0" : "") +
              RegExp.$2;
    ...
    function validateSettings() {
      ...
      settings.fonts.clockOuterCircleFontSize = parseInt(
        settings.fonts.clockOuterCircleFontSize
      );
      settings.fonts.clockInnerCircleFontSize = parseInt(
        settings.fonts.clockInnerCircleFontSize
      );
      ...
      if (!settings.separator || ("" + settings.separator).match(/[0-9]+/)) {
        console.error(
          "%c[jquery-clock-timepicker] Invalid separator specified: " +
            (settings.separator ? settings.separator : "(empty)") +
      ...
      if (
        ...
        (settings.minimum == settings.maximum ||
          !isTimeSmallerOrEquals(settings.minimum, settings.maximum))
      ) {
        console.log(
